...
#
# This script runs to create a Dockerfile for a new Drupal Console version.
# If you specify a partial version, like '1' or '1.0', it will determine the most recent sub version like 1.0.0-alpha2.
set -eo pipefail
...
function writeFiles {
	...
	sed -r -i -e 's/^(ENV DRUPAL_CONSOLE_VERSION) .*/\1 '"$fullVersion"'/' "$targetDir/Dockerfile"
}

tags="$(git ls-remote --tags https://github.com/hechoendrupal/DrupalConsole.git | cut -d/ -f3 | cut -d^ -f1 | cut -dv -f2 | sort -rV)"
